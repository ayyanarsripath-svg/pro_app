...
/// Sales Bill) described in spec sections 24-27, plus the supplier order
/// note PDF used by the daily-order WhatsApp auto-send reminder. Every bill:
...
class PdfService {
  ...
  // -----------------------------------------------------------------
  // SUPPLIER ORDER NOTE - daily-order WhatsApp auto-send reminder.
  // Deliberately plain (no bill number / payment box - it's a request to
  // the supplier, not a bill) so it prints clean on a phone screen too.
  // -----------------------------------------------------------------
  Future<Uint8List> buildOrderNotePdf({
      ...
      pw.Page(
        ...
        build: (context) {
          return pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              _header(shop, logo, 'SUPPLIER ORDER'),
              pw.SizedBox(height: 4),
              pw.Text('Date: ${formatDateTime(orderDate)}', style: const pw.TextStyle(fontSize: 9)),
              pw.SizedBox(height: 6),
              _box('TO SUPPLIER', [
                _kv('Name', supplierName),
                _kv('Phone', supplierPhone),
              ]),
              _box('ITEMS REQUIRED', [
              ...
              pw.Text(
                'Please arrange the above and share pricing / availability. Thank you.',
  ...
  /// Saves PDF bytes to a real file the OS/other apps (WhatsApp share
  /// sheet) can read, and returns its path. Uses the app's own documents
  /// directory - fine for a short-lived "about to share" file since it
  /// doesn't need to be a permanent record like a backup.
  Future<File> saveToFile(Uint8List bytes, String fileName) async {
    final dir = await getApplicationDocumentsDirectory();
    final ordersDir = Directory(p.join(dir.path, 'orders'));
    if (!await ordersDir.exists()) await ordersDir.create(recursive: true);
    final file = File(p.join(ordersDir.path, fileName));
